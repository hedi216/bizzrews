...
Business logos and Experience images use the centralized media API. Local development stores JPEG, PNG, and WebP uploads under ignored `.data/media` using randomized storage keys; uploads are limited to 8 MB and public responses expose stable API URLs rather than filesystem paths. Set `MEDIA_STORAGE_PROVIDER=LOCAL` and optionally `MEDIA_LOCAL_ROOT` in the ignored root `.env`. `PUBLIC_API_URL` controls absolute media URLs. Production intentionally refuses local storage and requires a durable object-storage adapter before deployment; no provider or credentials are committed.
