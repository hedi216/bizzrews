...
The Experience Builder separates Overview, Booking form, Page design, Availability, and Preview & Publish. New Experiences start with a Hero and one booking-form section. Business users edit customer-facing question labels, validation, and choices without handling internal keys or numeric positions; stable keys remain available only under Advanced. Published revisions and all of their fields, blocks, options, and media placements remain immutable, while creating the next draft clones the complete presentation with new configuration IDs.

Business logos and Experience images use the centralized media API. Local development stores JPEG, PNG, and WebP uploads under the repository-root ignored `.data/media` directory using randomized storage keys, regardless of which workspace launches the API. A blank `MEDIA_LOCAL_ROOT` uses that deterministic default; a nonblank value explicitly overrides it. Uploads are limited to 8 MB and public responses expose stable API URLs rather than filesystem paths. Set `MEDIA_STORAGE_PROVIDER=LOCAL` in the ignored root `.env`. `PUBLIC_API_URL` controls absolute media URLs. Production intentionally refuses local storage and requires a durable object-storage adapter before deployment; no provider or credentials are committed.
